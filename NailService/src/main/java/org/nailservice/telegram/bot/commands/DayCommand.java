...
public class DayCommand implements Command {
    ...
    private List<InlineKeyboardButton> createDateRow(LocalDate date) {
        return createButtonsRow(createButton(date.toString(), String.format("/day %s", date.toString())));
    }
    ...
    private List<List<InlineKeyboardButton>> addOrders(List<List<InlineKeyboardButton>> rowList, LocalDate date) {
        ...
        for (Map.Entry<String, String> order: sheduleCreator.createSheduleTextForCustomer(shedule).entrySet()) {
    ...
    private List<InlineKeyboardButton> createCalendarRow(LocalDate date) {
        ...
        return createButtonsRow(createButton("Показать весь месяц", String.format("/calendar %s", month)));
    }
