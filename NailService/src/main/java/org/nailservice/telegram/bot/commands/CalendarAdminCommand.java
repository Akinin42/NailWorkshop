...
public class CalendarAdminCommand implements Command {
    ...
    private List<List<InlineKeyboardButton>> addDayRows(List<List<InlineKeyboardButton>> rowList, LocalDate date) {
        int day = 1;
        while (day <= getMonthLenght(date)) {
            int mondayPosition = LocalDate.of(date.getYear(), date.getMonth(), day).getDayOfWeek().getValue();
            rowList.add(createDaysRow(day, mondayPosition, date));
            day += (8 - mondayPosition);
        }
        return rowList;
    }

    private List<InlineKeyboardButton> createDaysRow(int firstRowNumber, int position, LocalDate date) {
        List<InlineKeyboardButton> daysRow = new ArrayList<>();
        int monthLength = getMonthLenght(date);
        for (int i = 1; i <= 7; i++) {
    ...
    private int getMonthLenght(LocalDate date) {
        return date.getMonth().length(Year.isLeap(date.getYear()));
    }
